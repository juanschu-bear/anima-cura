...
export async function GET(request: NextRequest) {
  ...
  if (patientId) {
    ...
    return NextResponse.json({
      ...
      messages: (messages || []).map(m => ({ ...m, sender: m.sender_type })),
    });
  }

  const { data: conversations } = await sc
    .from("patient_messages")
    .select("patient_id, text, sender_type, created_at")
  ...
  const latestByPatient: Record<string, { patient_id: string; last_message: string; last_sender: string; last_at: string; count: number }> = {};
  for (const msg of conversations) {
    if (!latestByPatient[msg.patient_id]) {
      latestByPatient[msg.patient_id] = { patient_id: msg.patient_id, last_message: msg.text?.slice(0, 80) || "", last_sender: msg.sender_type, last_at: msg.created_at, count: 0 };
    }
    latestByPatient[msg.patient_id].count++;
  }

  const patIds = Object.keys(latestByPatient);
  const { data: patients } = await sc.from("patients").select("id, vorname, nachname").in("id", patIds.length > 0 ? patIds : ["00000000-0000-0000-0000-000000000000"]);
...
export async function POST(request: NextRequest) {
  ...
  const sc = createServerClient();
  const { error } = await sc.from("patient_messages").insert({ patient_id, sender_type: "praxis", sender_name: "Praxis", text });
