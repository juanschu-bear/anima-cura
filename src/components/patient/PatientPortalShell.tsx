...
export default function PatientPortalShell({ patientName, patientId }: Props) {
  ...
  useEffect(() => {
    if (chatScrollRef.current && tab === "chat") {
  ...
  const s = {
    shell: { maxWidth: 430, margin: "0 auto", minHeight: "100vh", fontFamily: "'DM Sans',sans-serif", background: bg, color: fg, position: "relative" as const },
    card: { borderRadius: 18, padding: 22, marginBottom: 14, background: cardBg, border: "1px solid " + border },
    hd: { fontFamily: "'Fraunces',serif" },
    lb: { fontSize: 11, fontWeight: 700 as const, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: muted },
  };
    ...
    return (
      <div style={{ ...s.shell, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#22c55e", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 800, color: "#fff", ...s.hd, margin: "0 auto 16px" }}>A</div>
  ...
  const Header = (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 20px 0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 42, height: 42, borderRadius: "50%", background: "#22c55e", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: "#fff", ...s.hd }}>A</div>
        <span style={{ ...s.hd, fontSize: 22, fontWeight: 700 }}>Anima Cura</span>
      </div>
  ...
  const HintBanner = isOverdue ? (
    <div style={{ margin: "16px 20px 0", padding: "16px 18px", borderRadius: 16, display: "flex", gap: 14, alignItems: "flex-start", background: dk ? "rgba(80,20,15,0.4)" : "rgba(220,80,70,0.06)", border: "1px solid " + (dk ? "rgba(200,60,50,0.2)" : "rgba(200,60,50,0.12)") }}>
      ...
      <div style={{ fontSize: 14, lineHeight: 1.55, color: dk ? "#d4b0a8" : "#7a4a42" }}>
        <strong>Hinweis:</strong> Es gibt eine kleine Verz&ouml;gerung bei Ihrer letzten Rate. Bitte pr&uuml;fen Sie dies kurz im Fortschritt-Reiter, damit Ihr Verlauf reibungslos weitergeht.
      </div>
    </div>
  ) : null;
  ...
  const NotifsDD = nOpen ? (
    <div style={{ padding: "12px 20px 0" }}>
      {notifs.map(n => (
        <div key={n.id} style={{ padding: 14, borderRadius: 14, marginBottom: 8, display: "flex", gap: 12, alignItems: "flex-start", background: cardBg, border: "1px solid " + (!n.gelesen ? (dk ? "rgba(74,222,128,0.15)" : "rgba(34,197,94,0.12)") : border) }}>
  ...
  const HomeTab = (
    <div>
      ...
      <div style={{ padding: "16px 20px 0" }}>
        <p style={{ fontSize: 13, color: muted }}>Willkommen zur&uuml;ck</p>
        <h1 style={{ ...s.hd, fontSize: 24, fontWeight: 800 }}>Hallo, {firstName}</h1>
      </div>
      <div style={{ ...s.card, margin: "16px 20px 14px", background: dk ? "rgba(74,222,128,0.03)" : "rgba(34,197,94,0.03)", border: "1px solid " + (dk ? "rgba(74,222,128,0.1)" : "rgba(34,197,94,0.1)") }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
          <div>
            <div style={{ ...s.lb, color: grn, marginBottom: 4 }}>Aktuelle Phase</div>
            <div style={{ ...s.hd, fontSize: 19, fontWeight: 700 }}>{activePhase ? activePhase.name : "Keine Phase"}</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ flex: 1, borderRadius: 12, padding: "12px 8px", textAlign: "center", background: dk ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)", border: "1px solid " + border }}>
            <div style={{ ...s.lb, marginBottom: 3 }}>Investiert</div>
            <div style={{ ...s.hd, fontSize: 20, fontWeight: 800, color: grn }}>{pct}%</div>
          </div>
          <div style={{ flex: 1, borderRadius: 12, padding: "12px 8px", textAlign: "center", background: dk ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)", border: "1px solid " + border }}>
            <div style={{ ...s.lb, marginBottom: 3 }}>N&auml;chste Rate</div>
            <div style={{ ...s.hd, fontSize: 20, fontWeight: 800 }}>{dl > 0 ? dl + "T" : "Heute"}</div>
          </div>
          <div style={{ flex: 1, borderRadius: 12, padding: "12px 8px", textAlign: "center", background: dk ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)", border: "1px solid " + border }}>
            <div style={{ ...s.lb, marginBottom: 3 }}>Raten</div>
            <div style={{ ...s.hd, fontSize: 20, fontWeight: 800 }}>{rp ? rp.raten_bezahlt : 0}/{rp ? rp.raten_gesamt : 0}</div>
          </div>
        </div>
      </div>
      {badges.length > 0 && (
        <div style={{ padding: "0 20px", marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ ...s.hd, fontSize: 15, fontWeight: 700 }}>Erfolge</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: muted }}>{badges.filter(b => b.freigeschaltet).length}/{badges.length}</span>
          </div>
          <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 4 }}>
            {badges.map(b => (
              <div key={b.id} onClick={() => setPopup(b)} style={{ minWidth: 76, padding: "14px 8px", borderRadius: 14, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer", background: cardBg, border: "1px solid " + border, opacity: b.freigeschaltet ? 1 : 0.25 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, padding: "0 20px", marginBottom: 14 }}>
        ...
        <div onClick={() => setTab("more")} style={{ display: "flex", alignItems: "center", gap: 14, padding: 16, borderRadius: 16, cursor: "pointer", background: cardBg, border: "1px solid " + border }}>
          ...
          <div><div style={{ fontSize: 14, fontWeight: 700 }}>Dokumente</div><div style={{ fontSize: 12, color: muted }}>Vertr&auml;ge &amp; Pl&auml;ne</div></div>
        </div>
      </div>
      {tipps.length > 0 && (
        <div style={{ margin: "0 20px 14px", borderRadius: 14, padding: 16, background: dk ? "rgba(234,179,80,0.04)" : "rgba(234,179,80,0.05)", border: "1px solid " + (dk ? "rgba(234,179,80,0.1)" : "rgba(234,179,80,0.12)") }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: warn, marginBottom: 6 }}>{"\ud83d\udca1"} Tipp f&uuml;r deine Phase</div>
  ...
  const JourneyTab = (
    <div>
      ...
      {HintBanner}
      <div style={{ padding: "16px 20px 0" }}>
        <h1 style={{ ...s.hd, fontSize: 23, fontWeight: 800, marginBottom: 2 }}>Verlauf</h1>
      ...
      <div style={{ position: "relative", paddingLeft: 36, margin: "0 20px" }}>
        <div style={{ position: "absolute", left: 13, top: 8, bottom: 8, width: 2, borderRadius: 2, background: dk ? "#1a1a1a" : "#e0d8cc" }} />
        {phasen.map(ph => (
          <div key={ph.id} style={{ position: "relative", marginBottom: 20 }}>
            <div style={{ position: "absolute", left: -30, top: 8, width: 14, height: 14, borderRadius: "50%", background: ph.status === "abgeschlossen" ? grn : ph.status === "aktiv" ? purple : (dk ? "#222" : "#e0d8cc"), boxShadow: ph.status === "aktiv" ? "0 0 10px " + purple + "40" : "none" }} />
            <div style={{ ...s.card, margin: 0, marginBottom: 0 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <span style={{ ...s.hd, fontSize: 16, fontWeight: 700 }}>{ph.name}</span>
      ...
      {tipps.length > 0 && (
        <div style={{ padding: "12px 20px 0" }}>
          <p style={{ ...s.hd, fontSize: 15, fontWeight: 700, marginBottom: 12 }}>Pflegetipps</p>
          {tipps.map(t => (
  ...
  const ProgressTab = (
    <div>
      ...
      {HintBanner}
      <div style={{ padding: "16px 20px 0" }}>
        <h1 style={{ ...s.hd, fontSize: 23, fontWeight: 800, marginBottom: 2 }}>Dein Fortschritt</h1>
      ...
      <div style={{ textAlign: "center", padding: "0 20px 8px" }}>
        <div style={{ position: "relative", width: 200, height: 200, margin: "0 auto" }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 3 }}>
            <span style={{ ...s.hd, fontSize: 44, fontWeight: 800 }}>{pct}%</span>
            <span style={{ ...s.lb, color: grn, marginTop: 2 }}>INVESTIERT</span>
          </div>
        </div>
      </div>
      <div style={{ ...s.card, margin: "8px 20px 14px", display: "flex", padding: 18 }}>
        <div style={{ flex: 1, textAlign: "center" }}><div style={{ ...s.hd, fontSize: 18, fontWeight: 800, color: grn }}>{fmtEuro(rp ? rp.investiert : 0)}</div><div style={{ ...s.lb, marginTop: 2 }}>Investiert</div></div>
        <div style={{ width: 1, background: border }} />
        <div style={{ flex: 1, textAlign: "center" }}><div style={{ ...s.hd, fontSize: 18, fontWeight: 800 }}>{fmtEuro(rp ? rp.offen_betrag : 0)}</div><div style={{ ...s.lb, marginTop: 2 }}>Offen</div></div>
        <div style={{ width: 1, background: border }} />
        <div style={{ flex: 1, textAlign: "center" }}><div style={{ ...s.hd, fontSize: 18, fontWeight: 800 }}>{fmtEuro(rp && rp.plan ? rp.plan.gesamtbetrag : 0)}</div><div style={{ ...s.lb, marginTop: 2 }}>Gesamt</div></div>
      </div>
      {isOverdue && rp && rp.ueberfaellig && (
        <div>
          <div style={{ ...s.card, margin: "0 20px 14px", display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ flex: 1 }}>
              <div style={{ ...s.lb, color: red, marginBottom: 4 }}>RATE &Uuml;BERF&Auml;LLIG</div>
              <div style={{ ...s.hd, fontSize: 30, fontWeight: 800, marginBottom: 2 }}>{rp.ueberfaellig.betrag.toFixed(2).replace(".",",")} \u20ac</div>
            ...
            <div style={{ width: 80, borderRadius: 14, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "14px 0", background: dk ? "#1a1a1a" : "#f0ece4" }}>
              <div style={{ ...s.hd, fontSize: 28, fontWeight: 800 }}>{rp.raten_bezahlt}</div>
      ...
      {!isOverdue && rp && rp.naechste_rate && (
        <div style={{ ...s.card, margin: "0 20px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ ...s.lb, color: dl <= 5 ? warn : muted, marginBottom: 4 }}>N&auml;chste Rate</div>
            <div style={{ ...s.hd, fontSize: 26, fontWeight: 800 }}>{rp.naechste_rate.betrag} \u20ac</div>
      ...
      {pays.length > 0 && (
        <div>
          <div style={{ padding: "0 20px" }}><p style={{ ...s.hd, fontSize: 18, fontWeight: 800, marginBottom: 10 }}>Historie</p></div>
          <div style={{ ...s.card, margin: "0 20px 14px", padding: "2px 18px" }}>
            {pays.map((p, i) => (
              <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 0", borderTop: i > 0 ? "1px solid " + (dk ? "#1a1a1a" : "#f0e8dc") : "none" }}>
                ...
                <span style={{ ...s.hd, fontSize: 15, fontWeight: 700, color: grn }}>{p.betrag} \u20ac</span>
              </div>
            ))}
          </div>
        </div>
  ...
  const ChatTab = (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 90px)" }}>
      <div style={{ padding: "18px 20px 14px" }}>
        <h1 style={{ ...s.hd, fontSize: 21, fontWeight: 800 }}>Praxis Chat</h1>
      ...
      <div ref={chatScrollRef} style={{ flex: 1, overflowY: "auto", padding: "14px 20px" }}>
        {msgs.map(m => (
          <div key={m.id} style={{ display: "flex", marginBottom: 10, justifyContent: m.sender_type === "patient" ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: "78%", padding: "12px 16px", fontSize: 14, lineHeight: 1.5, borderRadius: 20, ...(m.sender_type === "patient" ? { background: "#22c55e", color: "#fff", borderBottomRightRadius: 4 } : { background: cardBg, border: "1px solid " + border, borderBottomLeftRadius: 4 }) }}>
      ...
      <div style={{ padding: "8px 18px", display: "flex", alignItems: "center", gap: 8, fontSize: 11, fontWeight: 500, background: dk ? "rgba(100,80,200,0.04)" : "rgba(100,80,200,0.03)", borderTop: "1px solid " + (dk ? "rgba(100,80,200,0.06)" : "rgba(100,80,200,0.06)"), color: purple }}>
        {"\ud83e\udd16"} iCura beantwortet h&auml;ufige Fragen sofort.
      </div>
      <div style={{ padding: "10px 16px", display: "flex", gap: 8, alignItems: "center", borderTop: "1px solid " + border, background: dk ? "rgba(0,0,0,0.95)" : "rgba(245,241,235,0.95)" }}>
        ...
        <input value={msgInput} onChange={e => setMsgInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") sendMsg(); }} placeholder="Nachricht schreiben..." style={{ flex: 1, padding: "11px 16px", borderRadius: 24, fontSize: 14, outline: "none", fontFamily: "inherit", background: dk ? "#111" : "#fff", border: "1px solid " + border, color: fg }} />
        <button onClick={sendMsg} style={{ width: 42, height: 42, borderRadius: "50%", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "#fff", background: msgInput.trim() ? "#22c55e" : (dk ? "#1a1a1a" : "#e8e2d8") }}>{"\u2191"}</button>
      </div>
    </div>
  );
  ...
  const MoreTab = (
    <div>
      {Header}
      <div style={{ padding: "16px 20px 0" }}>
        <h1 style={{ ...s.hd, fontSize: 23, fontWeight: 800, marginBottom: 16 }}>Mehr</h1>
      </div>
      {docs.length > 0 && <div style={{ padding: "0 20px" }}><p style={{ ...s.hd, fontSize: 15, fontWeight: 700, marginBottom: 10 }}>Dokumente</p></div>}
      {docs.map(d => (
        <div key={d.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "15px 16px", borderRadius: 14, margin: "0 20px 8px", cursor: "pointer", background: cardBg, border: "1px solid " + border }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div><div style={{ fontSize: 14, fontWeight: 600 }}>{d.name}</div><div style={{ fontSize: 11, color: muted }}>{fmtDate(d.hochgeladen_am)}</div></div>
          </div>
      ...
      <div style={{ borderRadius: 18, padding: 28, margin: "18px 20px", textAlign: "center", background: dk ? "rgba(100,80,200,0.06)" : "rgba(100,80,200,0.04)", border: "1px solid " + (dk ? "rgba(100,80,200,0.1)" : "rgba(100,80,200,0.08)") }}>
        ...
        <h3 style={{ ...s.hd, fontSize: 17, fontWeight: 700, marginBottom: 6 }}>Beratungsgespr&auml;ch</h3>
        <p style={{ fontSize: 13, lineHeight: 1.5, color: soft, marginBottom: 18 }}>Fragen zu deinem Ratenplan? Starte ein Videogespr&auml;ch mit unserem Praxisberater.</p>
        <button style={{ border: "none", borderRadius: 14, padding: "14px 32px", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", background: "#7c3aed" }}>Gespr&auml;ch anfragen</button>
      </div>
      <div style={{ ...s.card, margin: "0 20px 14px" }}>
        <p style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>&Uuml;ber diese App</p>
        <p style={{ fontSize: 12, color: muted, lineHeight: 1.6 }}>Anima Cura Patientenportal &mdash; Praxis Dr. Maria Schubert, Leipzig.</p>
      </div>
      <div style={{ padding: "0 20px 16px", textAlign: "center" }}>
        ...
        <p style={{ fontSize: 12, color: dk ? "#222" : "#ccc" }}>Datenschutz &middot; Impressum &middot; Kontakt</p>
      </div>
    </div>
  );
      ...
      {popup && (
        <div onClick={() => setPopup(null)} style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 32, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}>
          <div onClick={e => e.stopPropagation()} style={{ borderRadius: 24, padding: "36px 28px", textAlign: "center", maxWidth: 300, width: "100%", background: cardBg, border: "1px solid " + border }}>
            ...
            <h3 style={{ ...s.hd, fontSize: 22, fontWeight: 700, marginBottom: 8 }}>{popup.titel}</h3>
            ...
            <button onClick={() => setPopup(null)} style={{ display: "block", margin: "18px auto 0", background: "none", border: "none", fontSize: 14, fontWeight: 600, cursor: "pointer", color: soft }}>Schlie&szlig;en</button>
          </div>
        </div>
      )}
      <style>{fontCss}</style>
