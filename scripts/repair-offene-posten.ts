...
type MatchedCandidate = {
  ...
  ref: ReturnType<typeof buildReferenceResult>;
};
...
async function fetchAllCandidates(db: ReturnType<typeof createServerClient>): Promise<TxRow[]> {
  const all: TxRow[] = [];
  let from = 0;
  const size = 1000;
...
async function fetchOpenPosten(
  db: ReturnType<typeof createServerClient>
): Promise<Map<string, OffenerPostenRow>> {
...
function buildReferenceResult(tx: TxRow, posten: OffenerPostenRow) {
  ...
  return {
    ...
    posten_id: posten.id,
    status: (ueberzahlung > 0 && !posten.patient_id) ? "abweichung" as const : "auto" as const,
    details: {
      name_score: 0,
      betrag_match: diff === 0,
      zweck_score: 100,
      methode: "referenz" as const,
...
async function clearPlaceholderMatches(
  db: ReturnType<typeof createServerClient>,
  dryRun: boolean
) {
  const { data, error } = await db
